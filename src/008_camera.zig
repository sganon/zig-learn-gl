...
const shd = @import("shaders/008_camera.glsl.zig");
...
export fn frame() void {
    ...
    for (0.., cubePositions) |i, pos| {
        ...
        const radius = 10.0;
        const camX = std.math.sin(time) * radius;
        const camZ = std.math.cos(time) * radius;
        const view = math.Mat4.lookat(.{ .x = camX, .y = 0.0, .z = camZ }, .{ .x = 0.0, .y = 0.0, .z = 0.0 }, .{ .x = 0.0, .y = 1.0, .z = 0.0 });
